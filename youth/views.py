...
def EMAIL_JOB(request):
    ...
    return render(request,'email_job.html')

def EMAIL_VOLUNTEER(request):
    li =[]
    em2 = VOLUNTEERS.objects.all()
    for i in em2:
        li.append(i.email)
    ...
    return render(request,'email_volunteer.html')

def EMAIL_NYP_UNIT(request):
    li =[]
    em1 = Member.objects.all()
    em2 = Groupadmin.objects.all()
    for i in em1:
    ...
    return render(request,'email_NYP_unit.html')
